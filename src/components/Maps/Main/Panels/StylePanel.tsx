...
const StylePanel: React.FC<StylePanelProps> = ({
  ...
  const handleButtonHover = (scale: number) => {
    gsap.to(".map-toggle-btn", { scale, duration: 0.1 });
  };
  ...
  return (
    <div className="absolute bottom-4 right-4 z-40">
      <button
        ...
        onMouseEnter={() => handleButtonHover(1.2)}
      ...
      {isPanelOpen && (
        <div
          ref={panelRef}
          className="mt-3 p-5 bg-white dark:bg-gray-900 rounded-lg shadow-2xl origin-bottom-right"
        >
          ...
          <div className="space-y-3">
            {mapStyles.map((style) => (
              <div
                key={style.id}
                className={`p-3 flex items-center rounded-lg cursor-pointer transition-transform duration-300
                  ${
                    selectedStyle === style.style
                      ? "bg-primary text-white border-2 border-primary transform scale-105"
