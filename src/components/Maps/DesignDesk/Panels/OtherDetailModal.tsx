...
interface OtherDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  otherData: ObjectData | null;
  onSubmit: (
    updatedData: ObjectData & { City: string; Plan_Type: string }
  ) => void;
}

const OtherDetailModal: React.FC<OtherDetailModalProps> = ({
  isOpen,
  onClose,
  otherData,
  onSubmit,
  ...
  useEffect(() => {
    if (otherData) {
      const matchingOther = others.find(
        (item) =>
          item.Component_ID === parseInt(otherData.ID) &&
          item.Type === otherData.Type
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const updatedData: ObjectData & { City: string; Plan_Type: string } = {
        Lat: otherData?.Lat || 0,
        Long: otherData?.Long || 0,
        Type: otherData?.Type || "",
        ID: otherData?.ID || "",
        Chain_ID: otherData?.Chain_ID || 0,
        City,
        Name: name,
        Plan_Type: planType,
      };
